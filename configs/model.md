...
| kid           | 唯一标识       | 字符串   | usr(48),   org(32),    app(24)                      | varchar(64) NOT NULL, udx_user_kid                   |
| type          | 账户类型       | 字符串   | usr(用户), org(租户), app(应用)                     | varchar(64) NOT NULL DEFAULT 'usr', idx_user_type    |
| name          | 用户名         | 字符串   |                                                     | varchar(64) NOT NULL, idx_user_name                  |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| status        | 状态           | 数值     | 1:启用 0:禁用 2: 未激活 3:作废                      | tinyint(4) DEFAULT 1                                 |
...
| org_cod       | 租户ID         | 字符串   |                                                     | varchar(64) NOT NULL, udx_org_user_kid,udx_org_user_ocd,udx_org_user_name,fk_org_user_ocd->organization.code |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
...
---
## 策略服务实体(`policy_service`)

服务是平台提供的服务入口， 与client是多对一关系， 一个服务分裂出多个client为不同的租户提供服务， 但是他们之间只是通过policy_service_audience中的audience
形成关联关系
...
| id            | 唯一标识       | 数值     |                                                     | int(11) NOT NULL AUTO_INCREMENT, primary             |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| name          | 服务名称       | 字符串   |                                                     | varchar(64), udx_policy_service_name                 |
| code          | 服务编码       | 字符串   |                                                     | varchar(64), udx_policy_service_code                 |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
...
---
## 策略服务实体(`policy_service_org`)

租户被授权的服务列表

| 字段          | 中文说明       | 字段类型 | 备注                                                | MYSQL                                                |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| svc_id        | 服务标识       | 数值     |                                                     | int(11) NOT NULL, fk_policy_service_sid->policy_service.id |
| org_cod       | 租户标识       | 字符串   |                                                     | varchar(64) NOT NULL, fk_policy_service_orcd->organization.code |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| expired       | 授权有效期     | 时间格式 |                                                     | timestamp                                            |
...
---
## 策略服务受众实体(`policy_service_audience`)
...
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| id            | 唯一标识       | 数值     |                                                     | int(11) NOT NULL AUTO_INCREMENT, primary             |
| svc_id        | 服务标识       | 数值     |                                                     | int(11) NOT NULL, fk_policy_service_audience_sid->policy_service.id |
| org_cod       | 租户标识       | 字符串   |                                                     | varchar(64) NOT NULL, fk_policy_service_audience_ocd->organization.code |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| audience      | 受众域         | 字符串   | 绑定服务                                            | varchar(255), udx_policy_service_audience            |
| resource      | 受众源         | 字符串   | 绑定服务                                            | varchar(255), udx_policy_service_audience            |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| status        | 状态           | 数值     | 1:启用 0:禁用 2:未激活                              | tinyint(4) DEFAULT 1                                 |
| description   | 描述           | 字符串   |                                                     | varchar(255)                                         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| id            | 唯一标识       | 数值     |                                                     | int(11) NOT NULL AUTO_INCREMENT, primary             |
| svc_id        | 服务标识       | 数值     |                                                     | int(11), fk_policy_service_action_sid->policy_service.id |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
...
| 字段          | 中文说明       | 字段类型 | 备注                                                | MYSQL                                                |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| plcy_id       | 策略标识       | 数值     |                                                     | int(11), fk_policy_statement_plcy_id->policy.id      |
...
| 字段          | 中文说明       | 字段类型 | 备注                                                | MYSQL                                                |
| ------------- | -------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| token_bid     | 令牌归属       | 数值     |                                                     | int(11),     idx_token_bid                           |
| token_type    | 令牌类型       | 字符串   | 标记令牌的类型(account, platform)                   | varchar(32), idx_token_bid                           |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| org_cod       | 租户标识       | 字符串   | 如果不为空,表示账户和租户绑定                       | varchar(64), fk_token_org_cod->organization.code     |
